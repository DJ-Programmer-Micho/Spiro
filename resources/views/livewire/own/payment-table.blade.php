<div>
    @include('livewire.own.payment-form')
    <div class="m-4">
        <h2 class="text-lg font-medium mr-auto">
            <b style="color: #31fbe2">{{__('PAYMENT TABLE')}}</b>
        </h2>
        <div class="row d-flex justify-content-between m-0">
            ...
            <div>
                <div class="">
                    <button class="btn btn-info" data-toggle="modal" data-target="#createPaymentModal">{{__('Add New Payment')}}</button>
                </div>
            </div>
        </div>
            ...
            <table class="table table-striped table-hover table-sm table-dark">
                ...
                <tbody>
                    ...
                    <tr style="font-size: 14px">
                        ...
                        @endforeach
                        <td>
                            <button type="button" data-toggle="modal" data-target="#updatePaymentModal"
                                wire:click="editPayment({{ $item->id }})" class="btn btn-primary m-1">
                                <i class="far fa-edit"></i>
                            </button>
                            <button type="button" data-toggle="modal" data-target="#deletePaymentModal"
                                wire:click="deletePayment({{ $item->id }})" class="btn btn-danger m-1">
