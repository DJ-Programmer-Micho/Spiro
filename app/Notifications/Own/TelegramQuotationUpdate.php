...
class TelegramQuotationUpdate extends Notification
{
    ...
    public function toTelegram($notifiable)
    {
        ...
        if ($this->status !== $this->old_quotation_data['status']) {
            $content .= "*" . 'Grand Total Changed: '. $this->old_quotation_data['status'] . ' ➡️ ' . $this->status . "*\n";
        }

        if ($this->quotationStatus !== $this->old_quotation_data['quotation_status']) {
            $content .= "*" . 'Grand Total Changed: '. $this->old_quotation_data['quotation_status'] . ' ➡️ ' . $this->quotationStatus . "*\n";
        }
